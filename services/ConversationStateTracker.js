...
class ConversationStateTracker {
    ...
    detectAcknowledgment(message) {
        ...
        // Negation patterns
        const negations = ['no', 'nope', 'not that', 'not that date', 'no thanks', 'no need', 'wrong'];
        ...
        const affirmations = ['yes', 'yeah', 'yup', 'sure', 'okay', 'ok', 'alright', 'correct', 'right'];
        ...
        const relationships = ['father', 'dad', 'mother', 'mom', 'uncle', 'aunt', 'brother', 'sister', 'friend', 'partner', 'husband', 'wife', 'son', 'daughter'];
        if (relationships.some(rel => lower.includes(rel))) {
            return { type: 'affirmation', value: 'user_confirmed_relationship', confidence: 0.85, relationship: lower };
        }

        // Continuation patterns
        const continuations = ['tell me more', 'go on', 'continue', 'what else', 'and then'];
        if (continuations.some(cont => lower.includes(cont))) {
            return { type: 'continuation', value: 'user_wants_more', confidence: 0.85 };
        }

        // Clarification patterns
        const clarifications = ['i mean', 'actually', 'i meant', 'to clarify'];
        if (clarifications.some(clar => lower.includes(clar))) {
            return { type: 'clarification', value: 'user_clarifying', confidence: 0.8 };
        }
    ...
    detectIntent(message) {
        ...
        if ((lower.includes('his') || lower.includes('her') || lower.includes('their') || lower.includes('for my')) &&
            (lower.includes('horoscope') || lower.includes('chart') || lower.includes('reading') || lower.includes('about'))) {
    ...
    detectTopic(message, previousState = {}) {
        ...
        // Horoscope-related
        if (lower.includes('horoscope') || lower.includes('stars') ||
            lower.includes('cosmic') || lower.includes('astrology')) {
            return 'horoscope_request';
        }

        // Date-related
        if (this.detectDates(message).length > 0 || lower.includes('born on') ||
            lower.includes('birthday') || lower.includes('birth date')) {
            return 'discussing_dates';
        }

        // General chat
        if (lower.includes('how are') || lower.includes('what\'s up') ||
    ...
    async getState(chatId) {
        try {
            ...
            if (thread.last_entities) {
                try {
                    entities = typeof thread.last_entities === 'string'
    ...
    formatStateForAI(state) {
        ...
        if (state.last_acknowledgment) {
            ...
            parts.push(ackMap[state.last_acknowledgment] || state.last_acknowledgment);
        }

        if (state.dates && state.dates.length > 0) {
            const lastDate = state.dates[state.dates.length - 1];
            if (lastDate.isAmbiguous) {
                parts.push(`Date format ambiguous: ${lastDate.original} (need clarification)`);
            }
        }
