...
class OnboardingHandler {
  ...
  getVibePrompt(sign, birthDate = null) {
    ...
    const intro = birthDate ? 
      `Perfect — your birthday is ${this.formatBirthDate(birthDate)}. ${sign ? `You're a ${sign}` : 'I detected your sign'}.` :
      `Perfect! ${signEmoji}`;
  ...
  validateDob(text) {
    ...
    if (!iso) {
      return { valid: false, error: `I couldn't parse that date. Please use DD/MM/YYYY format like 15/08/1999.` };
    }

    // Check for future dates
    const date = new Date(iso);
    if (date > new Date()) {
      return { valid: false, error: `That date is in the future! Please enter your actual birthday in DD/MM/YYYY format.` };
    }
    ...
    if (age > 120) {
      return { valid: false, error: `That date seems unlikely. Please check and re-enter your birthday in DD/MM/YYYY format.` };
    }

    // Validate leap year dates
    const [year, month, day] = iso.split('-').map(Number);
    if (month === 2 && day === 29 && !this.isLeapYear(year)) {
      return { valid: false, error: `${year} wasn't a leap year, so February 29 doesn't exist for that year.` };
    }
  ...
  async handleStructuredDob(text, user) {
    try {
      ...
      return `Almost there — send the ${missing.join(' and ')} (e.g. '${missing[0]} ${missing[0] === 'day' ? '15' : missing[0] === 'month' ? '08' : '1999'}').`;
    } catch (error) {
  ...
  // Enhanced vibe handler that accepts text or emoji
  ...
  //     const emoji = Object.keys(this.vibes).find((e) => text.includes(e));
  //     if (emoji) {
  //       const style = this.vibes[emoji];
  //       await this.saveVibeAndAdvance(user, style);
  //       return `Love it! ✨\n\nFinal step: What should I call you? (Or say 'skip' if you prefer)`;
  //     }

  //     // NEW: get personality version (if service is wired)
  //     let patch = {
  //       preferred_conversation_style: style,
  //       stage: "get_name",
  //     };

  //     if (this.personality) {
  //       const profile = this.personality.getProfile(style);
  //       if (profile?.meta?.version) {
  //         patch.personality_version = profile.meta.version;
  //       }
  //     }

  //     // Check for text-based vibe selection
  //     const detectedVibe = this.detectVibeFromText(text);
  //     if (detectedVibe) {
  //       await this.saveVibeAndAdvance(user, detectedVibe);
  //       return `Love it! ✨\n\nFinal step: What should I call you? (Or say 'skip' if you prefer)`;
  //     }
...
detectVibeFromText(text) {
  ...
  //   // Use personality service if available
  //   if (this.personality) {
  //     try {
  ...
  //       });
  //     } catch (error) {
  //       this.logger.warn('Personality service failed, continuing without:', error);
  //       await this.safeDbUpdate(user.chat_id, {
  //         preferred_conversation_style: style,
  //         stage: 'get_name'
  //       });
...
async saveVibeAndAdvance(user, style) {
  ...
  async handleName(text, user) {
    try {
      ...
      if (!cleaned) {
        ...
        return `Final step: What should I call you? (Or say 'skip')`;
      }

      if (/^skip$/i.test(cleaned)) {
        await this.safeDbUpdate(user.chat_id, { stage: 'get_time' });
        return `No worries — we'll use a nickname later.\n\nOne last thing (optional): Do you know your birth time? Even a rough guess works (e.g., 'around 5pm' or '17:00'). If you don't know, say 'skip'.`;
      }

      const name = this.extractName(cleaned);
      if (!name) {
        return `That doesn't look like a name. Try something simple like "Asha" or "Rahul" — or say 'skip'.`;
      }
  ...
  async handleBirthTime(text, user) {
    try {
      if (!text) {
        await this.safeDbUpdate(user.chat_id, { stage: 'get_time' });
        return `Do you know your birth time? e.g., '5:30 pm', '17:00', 'around 7am'. Say 'skip' to finish without it.`;
      }
      ...
      if (!parsed) {
        return `I couldn't understand that time. Try '5pm', '5:30 pm', '17:00', 'around 8am', or say 'skip'.`;
      }
  ...
  finalSuccessMessage(user) {
    ...
    if (this.personality && user.preferred_conversation_style) {
      try {
        const personalizedWelcome = this.personality.generateWelcome(
  ...
  async incrementDobAttempts(user) {
    try {
      ...
      if (attempts >= 2) {
        ...
        this.trackEvent('dob_fallback_triggered', { userId: user.chat_id, attempts });
      } else {
        await this.safeDbUpdate(user.chat_id, { 
          dob_attempts: attempts
        });
